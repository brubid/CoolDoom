...
long GetFileLength(FILE* handle)
{
...
void W_AddFile (char *filename)
{
   ...
   // if ( (handle = open (filename,O_RDONLY | O_BINARY)) == -1)
	...
	singleinfo.size = LONG(GetFileLength(handle));
...
//	read (handle, fileinfo, length);
    ...
    printf("Number of lumps in IWAD: %i\n",numlumps );
...
int startPos = (int) ftell(l->handle);
  //  c = read (handle, dest, l->size);
 c= fread(dest,l->size,1,handle);
    int endPos = (int) ftell(l->handle);
 c=endPos-startPos;
//printf("C: %i \n", l->size);
    if (c < l->size)
...
void W_FreeLumpNumCache(int lump)
{
...
/*
	...
	*/
    ...
    if (!lumpcache[lump] || !((memblock_t*)lumpcache[lump])->user)
